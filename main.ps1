...
# ============================================================
# [D0] GFX ConVar — GTA5 / FiveM Graphics Quality Settings
#      ปรับได้แต่ละค่า: เปลี่ยน $GFX_* ด้านล่างตามต้องการ
# ============================================================
#
#  ค่าที่ใช้ได้:
#    GFX_QUALITY_OFF      = 0   ← ปิดสุด / FPS สูงสุด
#    GFX_QUALITY_NORMAL   = 1   ← กลาง
#    GFX_QUALITY_HIGH     = 2   ← สูง
#    GFX_QUALITY_VERY_HIGH = 3  ← สูงมาก (ต้องการ GPU แรง)
#
#  FPS ที่ได้กลับมาโดยประมาณ (เทียบกับ Ultra):
#    SHADOW_QUALITY    OFF = +25 FPS
#    GRASS_QUALITY     OFF = +10 FPS
#    REFLECTION_QUAL   OFF = +20 FPS
#    SHADOW_DISTANCE   OFF = +10 FPS
#    POSTFX            OFF = +7  FPS
#    SHADER_QUALITY    OFF = +5  FPS
#    PARTICLE_QUALITY  OFF = +5  FPS
#    WATER_QUALITY     OFF = +3  FPS
#    LOD_SCALE         0   = +10 FPS (ยิ่งต่ำยิ่ง detail น้อย)
#    SHADOW_SOFTNESS   0   = +5  FPS
#    FOG_VOLUMES       0   = +5  FPS
#    REFLECTION_BLUR   0   = +5  FPS (เดิม 92)
#    REFLECTION_MSAA   0   = ปิด
#    TESSELLATION      0   = +3  FPS
#
# ---- แก้ค่าด้านล่างนี้ได้เลย ----

$GFX_SHADOW_QUALITY    = 0   # 0=OFF(+25fps)  1=Normal  2=High  3=VeryHigh
$GFX_GRASS_QUALITY     = 0   # 0=OFF(+10fps)  1=Normal  2=High  3=VeryHigh
$GFX_REFLECTION_QUAL   = 0   # 0=OFF(+20fps)  1=Normal  2=High  3=VeryHigh
$GFX_SHADOW_DISTANCE   = 0   # 0=OFF(+10fps)  1=Normal  2=High  3=VeryHigh
$GFX_POSTFX            = 0   # 0=OFF(+7fps)   1=Normal  2=High  3=VeryHigh
$GFX_SHADER_QUALITY    = 0   # 0=OFF(+5fps)   1=Normal  2=High  3=VeryHigh
$GFX_PARTICLE_QUALITY  = 0   # 0=OFF(+5fps)   1=Normal  2=High  3=VeryHigh
$GFX_WATER_QUALITY     = 0   # 0=OFF(+3fps)   1=Normal  2=High  3=VeryHigh
$GFX_LOD_SCALE         = 0   # 0=ต่ำสุด(+10fps)  ยิ่งสูงยิ่ง detail ไกล
$GFX_SHADOW_SOFTNESS   = 0   # 0=OFF(+5fps)   ยิ่งสูงเงายิ่งนุ่ม
$GFX_FOG_VOLUMES       = 0   # 0=OFF(+5fps)   1=เปิด
$GFX_REFLECTION_BLUR   = 0   # 0=OFF(+5fps)   92=Default  ยิ่งสูงยิ่งสวย
$GFX_REFLECTION_MSAA   = 0   # 0=OFF          2/4/8=เปิด MSAA บน reflection
$GFX_TESSELLATION      = 0   # 0=OFF(+3fps)   1=Normal  2=High  3=VeryHigh

# แปลงเป็น string สำหรับเขียนลง CitizenFX registry
$gfxMap = @{
    "GFX_TARGET_SHADOW_QUALITY"    = [string]$GFX_SHADOW_QUALITY
    "GFX_TARGET_GRASS_QUALITY"     = [string]$GFX_GRASS_QUALITY
    "GFX_TARGET_REFLECTION_QUAL"   = [string]$GFX_REFLECTION_QUAL
    "GFX_TARGET_SHADOW_DISTANCE"   = [string]$GFX_SHADOW_DISTANCE
    "GFX_TARGET_POSTFX"            = [string]$GFX_POSTFX
    "GFX_TARGET_SHADER_QUALITY"    = [string]$GFX_SHADER_QUALITY
    "GFX_TARGET_PARTICLE_QUALITY"  = [string]$GFX_PARTICLE_QUALITY
    "GFX_TARGET_WATER_QUALITY"     = [string]$GFX_WATER_QUALITY
    "GFX_TARGET_LOD_SCALE"         = [string]$GFX_LOD_SCALE
    "GFX_TARGET_SHADOW_SOFTNESS"   = [string]$GFX_SHADOW_SOFTNESS
    "GFX_TARGET_FOG_VOLUMES"       = [string]$GFX_FOG_VOLUMES
    "GFX_TARGET_REFLECTION_BLUR"   = [string]$GFX_REFLECTION_BLUR
    "GFX_TARGET_REFLECTION_MSAA"   = [string]$GFX_REFLECTION_MSAA
    "GFX_TARGET_TESSELLATION"      = [string]$GFX_TESSELLATION
}

$gfxBase = "HKCU:\SOFTWARE\CitizenFX\GFX"
If (-not (Test-Path $gfxBase)) { New-Item -Path $gfxBase -Force | Out-Null }
ForEach ($kv in $gfxMap.GetEnumerator()) {
    Set-ItemProperty -Path $gfxBase -Name $kv.Key -Value $kv.Value -Type String -EA SilentlyContinue
}
